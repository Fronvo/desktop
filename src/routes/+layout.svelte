<script lang="ts">
	...
	import { dropdownPosition, loginSucceeded, modalVisible } from 'stores/app/main';
	import {
		darkTheme,
		initSocket,
		sessionAttached,
	...
	onMount(() => {
			...
			if (getKey('maxOnlineTime') && $showLayout) {
				setInterval(() => {
					...
					if (getKey('maxOnlineTime')) {
						if ($sessionTime >= getKey('maxOnlineTime') && !$sessionWarningShown) {
							// Set stats info
		...
		document.addEventListener('mousemove', (ev) => {
			dropdownPosition.set([Math.min(ev.clientX, window.innerWidth - 150), ev.clientY]);
		});
	});
