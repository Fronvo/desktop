<script lang="ts">
	...
	import { followModalForFollowing, followModalInfo, modalAnimDuration } from 'stores/app/main';
			...
			<div class="following-items-container">
				{#each followInfo as { username, following, followers, avatar, isFollower, isPrivate, isSelf }, i}
					<div on:click={() => viewProfile(i)}>
						...
						<h1 id="following">
							<span>
								{isFollower || isSelf || !isPrivate ? following.length : '?'}
							</span>
							following
						</h1>
						<h1 id="followers">
							<span>
								{isFollower || isSelf || !isPrivate ? followers.length : '?'}
							</span>
							followers
						</h1>
					</div>
