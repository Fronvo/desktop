<script lang="ts">
	...
	function isOwner(profileId?: string): boolean {
		return $joinedCommunity.ownerId == (profileId ? profileId : $ourProfileData.profileId);
	}
		...
		<div class="members-items-container">
			{#each memberInfo as { profileId, username, following, followers, avatar, isFollower, isPrivate, isSelf }, i}
				...
				>
					...
					<h1 id="following">
						<span>{isFollower || isSelf || !isPrivate ? following.length : '?'}</span>
						following
					</h1>
					<h1 id="followers">
						<span>{isFollower || isSelf || !isPrivate ? followers.length : '?'}</span>
						followers
					</h1>
				</div>
			{/each}
