<script lang="ts">
	...
	import type { CommunityMessage, CommunityMessageFinal } from 'interfaces/app/communities';
	...
	async function loadMessages(): Promise<void> {
		...
		function checkLoadingDone(): void {
			if (tempFinalizedMessages.length == $targetCommunityMessages.length) {
	...
	function deleteMessage(messageIndex: number): void {
		...
		for (const messageIndexLoop in $targetCommunityMessages) {
			...
			if (targetMessage.messageId == $targetCommunityMessages[messageIndex].messageId) {
	...
	function replyMessage(messageIndex: number): void {
		// Update reply info
		$replyingTo = findCachedData(finalizedMessages[messageIndex].ownerId).username;
	...
	function attachChatRequestListener(): void {
		socket.on('chatRequestUpdated', ({ accepted }) => {
			const contentInput = document.getElementById('textarea-content') as HTMLTextAreaElement;
	...
	function attachMemberChangeListener(): void {
		...
		socket.on('memberLeft', ({ profileId }) => {
			$joinedCommunity.members.splice($joinedCommunity.members.indexOf(profileId), 1);
		});
	}
	...
	onMount(() => {
		...
		unsubscribe = targetSendHeight.subscribe((newHeight) => {
			...
			chatContainer.style.marginBottom = `${$replyingTo ? newHeight + 50 : newHeight}px`;
		});
			...
			<div
				...
				in:fly={{
					...
					delay: !animationsFinished
						? Math.min((finalizedMessages.length - i) * 50, $maxChatAnimDelay)
						: 0,
				}}
			>
				<div class="message-info-container">
					...
					<div class="menu-container">
						...
						{#if $joinedCommunity?.ownerId == $ourProfileData.profileId}
							...
							<DeleteChatOption callback={() => deleteMessage(i)} />
						{/if}
					</div>
				</div>
				...
				{#if isReply}
					<div class="reply-container">
						{#if findMessageById(replyId)}
							<h1 id="reply-name">
								Replying to <span>
									{findCachedData(findMessageById(replyId).ownerId).username}
								</span>
							</h1>
							<h1 id="reply-message">
